...
//   e.preventDefault(); //  for no rifresh
//   if (validateInputs()) {
//     let userinfo = {
...
//     const isRegesterdUSer = users.filter(
//       (user) => user.email == document.getElementById("email").value
//     );
//     if (!isRegesterdUSer.length) {
//       users.push(userinfo);
...
//     seterror(
//       "#password",
//       "Password must contain at least 8 characters, 1 capital letter, 1 special character"
//     );
//   } else {
//     setsuccess("#password");
//     validPassword = true;
...
//     seterror("#confirmPassowrd", "password dosen't match");
//   } else {
...
form.addEventListener("submit", (e) => {
  ...
  if (validateInputs()) {
    ...
    if (!isRegisteredUser.length) {
      ...
      window.location.href = "register-message.html";
    } else {
      alert("User is already registered");
      return;
    }
